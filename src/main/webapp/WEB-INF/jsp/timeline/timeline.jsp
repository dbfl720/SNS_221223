...
<div class="d-flex justify-content-center">
	<div class="contents-box">
			...
			<div class="write-box border rounded m-3">
				<div class="textareaBox rounded border">  <%-- border: 테투리 색깔 연하게. --%>
					 <textarea id="writeTextArea" class="textareaTimeline w-100"   placeholder="내용을 입력해주세요."></textarea>
					 	<div class="file-upload d-flex justify-content-between">
						 	<div class="d-flex">
						 		 <%-- file태그는 숨겨두고 이미지를 클릭하면 file태그를 클릭한 것처럼 이벤트 줌. --%>
						 		 <input type="file" id="file"   multiple='multiple' class="d-none" accept=".gif, .jpg, .png, .jpeg , .webp">
							...
							<button id="writeBtn" type="button" name="timelineBtn" class="btn btn-light col-2">게시</button>
						</div>
				</div>
		    </div>
	...
	$("#file").on('change', function(e) { // e가 파일객체들고 있음.
		...
		$("#writeBtn").on('click', function() {
			...
			// 파일이 업로드 된 경우에만 확장자 체크
				...
				// 확장자만 뽑아서 소문자로 변경한다.
				if ($.inArray(ext, ['jpg', 'jpeg', 'png', 'gif', 'webp']) == -1) {
					alert("이미지 파일만 업로드 할 수 있습니다.");
					$('#file').val('');  // 파일을 비운다.
					return;
				}
